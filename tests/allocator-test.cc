...
namespace
{
	...
	__noinline void test_preflight()
	{
		...
		/*
		 * Flush quarantine now, so that the 16 byte free we're about to do
		 * isn't likely to trigger the revoker.  Do this after allocating, so
		 * that we're sure the heap allocator has initialized itself.
		 *
		 * TODO: Ideally, we'd have a way of probing at the epoch counter or
		 * deferring revocation (for use in testing) but this is probably
		 * sufficient in all but the most corner of corner cases.
		 */
		t = {5};
		TEST_EQUAL(heap_quarantine_flush(&t),
		           0,
		           "Failure to flush quarantine in preflight");

		/*
		 * Call free in assembler (pardon the noise that's usually generated by
		 * the compiler) and check that the return registers are both untagged.
		 */
		{
			int res, pTag, ca1Tag;
			...
			TEST(!Capability{p}.is_valid_temporal(),
			     "Freed pointer still live; load barrier out of service?");
		}
