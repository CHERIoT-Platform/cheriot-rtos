...
struct Foo
{
	...
	struct Control : Bitpack<uint32_t>
	{
		 ...
		 * than one bit.
		 */
		// BITPACK_DEFINE_TYPE_NUMERIC(Scalar3, bool, 21, 22);
		// BITPACK_DEFINE_TYPE_ENUM_CLASS(Badenum, bool, 21, 22) { V = false };
	} control;
...
constexpr static Foo F1 = []() {
	 ...
	 */
	BITPACK_ENUM_QTYPE_OP(f.control, Flag1, =, Yes);
	...
	f.more.r2().alter([](Foo::More::ReusedT r2) {
		BITPACK_ENUM_OP(r2.f1(), =, D);
		return r2;
	});
	f.more.r2().alter([](Foo::More::ReusedT r2) {
		BITPACK_ENUM_OP(r2.f2(), =, B);
		return r2;
	});

	return f;
}();
static_assert(BITPACK_BY_QTYPE(F1.control, Scalar) == 0x7F);
...
__noinline static void test_volatile(volatile Foo *vfp)
{
	...
	BITPACK_QVAL_OP(vfp->control, =, Flag0::Yes);
...
static void test_static_asserts()
{
	...
	static_assert(
	  BITPACK_WRAP_OP(F1.control.view<Foo::Control::Scalar>(), ==, 0x7F));
	static_assert(BITPACK_WRAP_QTYPE_OP(F1.control, Scalar, <=>, 0x7E) ==
	              std::strong_ordering::greater);
...
static void test_maybe_better_ergonomics()
{
	...
	static_assert(BITPACK_MASKED_REL(MBE2, ==, Drive::Down));
}
