...
/**
 * SFINAE templates to test the availability of operator= overloads.
 *
 * If you're here to see how Bitpacks work, you can safely ignore this bit.
 */
...
struct Foo
{
	...
	struct Control : Bitpack<uint32_t>
	{
		...
		template<>
		constexpr FieldInfo field_info_for_type<Flag1>()
		{
			return {5, 5};
		}
	...
	struct More : Bitpack<uint32_t>
	{
		...
		struct ReusedT : Bitpack<uint8_t>
		{
			...
			template<typename Self>
			constexpr auto f1c(this Self &&self)
			{
				return self.template view<ReusedE, {0, 1, true}>();
			}

			template<typename Self>
			constexpr auto f2(this Self &&self)
			{
				return self.template view<ReusedE, {4, 5}>();
			}
		};

		template<typename Self>
		constexpr auto r1(this Self &&self)
		{
			return self.template view<ReusedT, {0, 7}>();
		}
...
static constexpr bool CanAssignFieldV =
  CanAssign<F, typename F::Field::Type>::value;
...
__noinline static void test_volatile(volatile Foo *vfp)
{
	...
	BITPACK_ENUM_QTYPE_ASSIGN(vfp->control, Flag0, Yes);
	...
	vfp->more.r2().alter(
	  [](auto r2) { return r2.f2().with(Foo::More::ReusedE::C); });
	 ...
	 */
	static_assert(!CanAssign<volatile Foo::More, volatile Foo::More>::value);
	// Instead, we have to perform an explicit read.
	static_assert(CanAssign<volatile Foo::More, Foo::More>::value);
	vfp->more = vfp->more.read();
}

static void test_static_asserts()
{
	static_assert(BITPACK_ENUM_EQUAL(C1.view<Foo::Control::Drive>(), Up));
	static_assert(BITPACK_ENUM_QTYPE_EQUAL(C1, Control::Flag0, No));
...
static void test_maybe_better_ergonomics()
{
	...
	static_assert(BITPACK_ENUM_EQUAL(MBE1.view<Drive>(), Up));
	static_assert(BITPACK_ENUM_UTYPE_EQUAL(MBE1, Drive, Up));

	static_assert((BITPACK_ENUM_UTYPE_WITH(MBE1, Drive, Both) <=> MBE1) ==
	              std::strong_ordering::greater);

	constexpr static auto MBE2 = std::get<1>(MBETops);
	static_assert(MBE2.raw() == 0b1'10);
}
