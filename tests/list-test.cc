...
namespace
{
	...
	struct LinkedObject
	{
		 ...
		 *
		 */
		__always_inline ObjectRing *toRing()
		{
			return &this->ring;
		}
	};
...
int test_list()
{
	...
	counter = 0;
	objects.untyped.search([&counter](LinkedObject::ObjectRing *&cell) {
		auto *toFree = LinkedObject::from_ring(cell);

		// This steps the iterator (which we've taken by reference) backwards
		// one position, so the next iteration visits toFree's next.
		cell = ds::linked_list::unsafe_remove(cell);
