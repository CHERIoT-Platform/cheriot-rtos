...
namespace
{
	...
	void test_barrier()
	{
		...
		TEST_EQUAL(ret,
		           -ETIMEDOUT,
		           "Barrier wait with no rendezvousing thread should time out");
		async([&]() {
	                                   ...
	                                   T                 &mutex)
	{
		...
		TEST_EQUAL(ret,
		           -ETIMEDOUT,
		           "Condition variable should have timed out (not signaled)");
		// We should have not reacquired the lock due to timeout, so acquire it
		// again.
		mutex.lock();
		async([&]() mutable {
			sleep(8);
