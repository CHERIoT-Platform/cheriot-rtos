...
namespace
{
	...
	class GuardWord
	{
		...
		void lock()
		{
			...
			Debug::Assert(high == 0, "Corrupt guard word");
			high = LockBit;
		}
		...
		void unlock()
		{
			...
			Debug::Invariant(res == 0, "futex_wake failed; possible deadlock");
		}
...
{
	auto *g = reinterpret_cast<GuardWord *>(guard);
	Debug::Assert(!g->is_initialised(), "Releasing uninitialized guard");
	Debug::Assert(g->is_locked(), "Releasing unlocked guard");
	g->set_initialised();
	g->unlock();
}
