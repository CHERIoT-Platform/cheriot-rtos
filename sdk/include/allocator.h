...
enum [[clang::flag_enum]] AllocatorPermission
{
	 ...
	 *
	 * TODO This could represent the permission to split a quota.
	 */
 ...
 * sub-quota object itself. The sub-quota (child) is an object allocated using
 * the parent quota, and so counts against the quota of the parent until a
 * corresponding call to `recombine_quota`.
 *
 ...
 *
 * This will return an AllocatorCapability that refers to the newly created
 * sub-quota on success.
 *
 * Returns `-EPERM` if `parent` is not a valid allocator capability or does not
 * have `AllocatorPermitFull` permissions, `-ENOMEM` if `parent` has
 * insufficient quota to accommodate the requested `size` plus the overhead of
 * the sub-quota object itself, `-EINVAL` if either  `timeout` is not a valid
 * timeout pointer or the internal allocation fails for any reason (including
 * timeout), or `-ECHILD` if the maximum number of extant sub-quotas has been
 * reached.
 *
