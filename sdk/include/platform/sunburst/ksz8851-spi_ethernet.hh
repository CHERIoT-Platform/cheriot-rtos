...
namespace Ksz8851
{
	...
	/**
	 * Abstract over details of hardware, mostly to do with pushing and pulling
	 * bytes across SPI.
	 *
	 * The interface exposed here is one of bracketed transfers.  Starting a
	 * transfer pulls the chip select active, ending one releases the chip
	 * select. If the SPI host is shared between devices, it is the
	 * responsibility of this interface to properly multiplex transfers.
	 *
	 * All methods herein must be presumed to put the current thread to sleep.
	 */
	template<typename T>
	concept IsProvider = requires(const T t) {
		/// Abort any transfer in progress and fully hardware reset the chip
		{ t.chip_reset() } -> std::same_as<void>;

		/**
		 * Begin a SPI transfer.
		 */
		{ t.spi_transfer_start() } -> std::same_as<void>;
		/**
		 * End an active SPI transfer.
		 */
		{ t.spi_transfer_end() } -> std::same_as<void>;

		/**
		 * SPI transmission functions
		 */
		requires requires(const uint8_t *outData, size_t outSize) {
			/**
			 * Write bytes to SPI, with a source buffer that may be on the
			 * stack.
			 *
			 * This method may return early, before all bytes have been sent, if
			 * the underlying SPI host has a way of staging the request.
			 */
			{ t.spi_transmit_bounce(outData, outSize) } -> std::same_as<void>;

			/**
			 * Write bytes to SPI and end the transfer.
			 * The provided buffer capability must be Global.
			 */
			{ t.spi_transmit_and_end(outData, outSize) } -> std::same_as<void>;
		};

		/**
		 * SPI reception functions
		 */
		requires requires(uint8_t *inData, size_t inSize) {
			/**
			 * Read bytes from SPI, with a destination buffer that may be on the
			 * stack.
			 */
			{ t.spi_receive_bounce(inData, inSize) } -> std::same_as<void>;

			/**
			 * Arrange for the SPI to discard some number of bytes.
			 *
			 * This method may return early, before the bytes have been
			 * received, if the underlying SPI host has a way of staging the
			 * request.
			 */
			{ t.spi_receive_discard(inSize) } -> std::same_as<void>;

			/**
			 * Read bytes from SPI and end the transfer.
			 * The provided buffer capability must be Global.
			 */
			{ t.spi_receive_and_end(inData, inSize) } -> std::same_as<void>;
		};

		/**
		 * Providers must also source an IRQ capability.
		 */
		requires requires(Timeout *timeout, uint32_t count) {
			/**
			 * Return the IRQ futex counter
			 */
			{ t.receive_interrupt_value() } -> std::same_as<uint32_t>;

			/**
			 * Complete any active interrupt and wait for the next after
			 * `count`. Should tail-call `futex_timed_wait(timeout, _, count)`.
			 */
			{
				t.receive_interrupt_complete(timeout, count)
			} -> std::same_as<int>;
		};
	};
	 ...
	 *
	 * This is parameterized over a Provider that handles details of the
	 * hardware interconnect on a given board or platform.
	 */
	template<template<typename> typename ProviderTemplate>
	class Driver
	{
		/**
		...
		using Provider = ProviderTemplate<Debug>;
		static_assert(IsProvider<Provider>);
		...
		enum class SpiCommand : uint8_t
		{
			...
			// from the SPI interface into its internal buffer, so it takes
			// single SPI transaction for the entire frame. It is unrelated to
			// whether SPI driver uses PIO or DMA for the SPI transaction.
			ReadDma  = 0b10,
			WriteDma = 0b11,
		};
		...
		[[nodiscard]] uint16_t register_read(RegisterOffset reg) const
		{
			...
			// +-------------------+--------+
			//
			// Note that the access is 32-bit since bit 1 & 0 of the address is
			// not included. KSZ8851 have 16-bit registers so byte enable is
			// used to determine which register to access from the 32 bits
			...
			provider.spi_transfer_start();
			provider.spi_transmit_bounce(bytes, sizeof(bytes));
			uint16_t val;
			provider.spi_receive_bounce(reinterpret_cast<uint8_t *>(&val),
			                            sizeof(val));
			provider.spi_transfer_end();
			return val;
		}
		...
		void register_write(RegisterOffset reg, uint16_t val) const
		{
			...
			provider.spi_transfer_start();
			provider.spi_transmit_bounce(bytes, sizeof(bytes));
			provider.spi_transmit_bounce(reinterpret_cast<uint8_t *>(&val),
			                             sizeof(val));
			provider.spi_transfer_end();
		}
		...
		/**
		 * Number of frames yet to be received since last interrupt
		 * acknowledgement.
		 */
		...
		Driver() : provider()
		{
			...
			// Check the chip ID. The last nibble is revision ID and can be
			// ignored.
			...
			// The frame data burst field in this register controls how many
			// data from a frame is read per DMA operation. The programmer's
			// guide has a 4 byte burst, but to reduce SPI transactions and
			// improve performance we choose to use single-frame data burst
			// which reads the entire Ethernet frame in a single SPI DMA.
			register_write(
			...
			// Programmer's guide have a step to set the chip in half-duplex
			// when negotiation failed, but we omit the step since non-switching
			// hubs and half-duplex Ethernet is rarely used these days.
		...
		/**
		 * This device does not have a unique MAC address and so users must
		 * provide a locally administered MAC address if more than one device is
		...
		{
			return provider.receive_interrupt_value();
		}
		                               ...
		                               uint32_t lastInterruptValue)
		{
			...
			return provider.receive_interrupt_complete(timeout,
			                                           lastInterruptValue);
		}
		...
		std::optional<Frame> receive_frame()
		{
			...
			for (; framesToProcess; framesToProcess--)
			{
				...
				uint16_t length =
				  ...
				  0xFFF;
				bool valid = (status & ReceiveFrameValid) &&
				             !(status & (ReceiveCrcError | ReceiveRuntFrame |
				                         ReceiveFrameTooLong | ReceiveMiiError |
				                         ReceiveUdpFrameChecksumStatus |
				                         ReceiveTcpFrameChecksumStatus |
				                         ReceiveIpFrameChecksumStatus |
				                         ReceiveIcmpFrameChecksumStatus));

				if (!valid)
				{
					DebugFrameDrops::log("Dropping frame with status: {}",
					                     status);
				...
				provider.spi_transfer_start();
				provider.spi_transmit_bounce(&cmd, 1);

				// Initial words are ReceiveFrameHeaderStatus and
				// ReceiveFrameHeaderByteCount which we have already know the
				// value.
				provider.spi_receive_discard(8);

				provider.spi_receive_and_end(receiveBuffer.get(), paddedLength);
				...
				// Remove all permissions except load.  This also removes
				// global, so that this cannot be captured.
		...
		bool send_frame(const uint8_t *buffer, uint16_t length, auto &&check)
		{
			...
			if (paddedLength > MaxFrameSize)
			{
				Debug::log("Frame size {} is larger than the maximum size",
				           length);
				return false;
			}
			...
			provider.spi_transmit_bounce(reinterpret_cast<uint8_t *>(&header),
			                             sizeof(header));

			provider.spi_transmit_and_end(transmitBuffer.get(), paddedLength);
		...
		void drop_error_frame()
		{
			...
			// Wait for confirmation of frame release before attempting to
			// process next frame.
