...
namespace IbexCSR
{
	...
	struct Cpuctrlsts : Bitpack<uint32_t>
	{
		...
		BITPACK_MEMBER_ADD_ENUM(Icache, uint8_t, 0, 0) {
			Disable = 0,
			Enable  = 1
		};

		/// Enable data-independent timing featuers, if RTL has set SecureIbex
		BITPACK_MEMBER_ADD_ENUM(DataIndependentTiming, uint8_t, 1, 1) {
			Disable = 0,
			Enable  = 1
		};

		/// Dispatch dummy instructions, if RTL has set SecureIbex
		BITPACK_MEMBER_ADD_ENUM(DataInstructionInsertion, uint8_t, 2, 2) {
			Disable = 0,
			Enable  = 1
		};

		/// Frequency of dummy instruction insertion, if RTL has set SecureIbex
		BITPACK_MEMBER_ADD_NUMERIC(DummyInstructionRate, uint8_t, 3, 5);
		 ...
		 */
		BITPACK_MEMBER_ADD_ENUM(SynchronousException, uint8_t, 6, 6) {
			Inactive = 0,
			Active   = 1
		};
		 ...
		 */
		BITPACK_MEMBER_ADD_ENUM(DoubleFault, uint8_t, 7, 7) {
			Unseen = 0,
			Seen   = 1
		};
	};
