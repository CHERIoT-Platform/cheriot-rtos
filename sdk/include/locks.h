 ...
 *
 * Returns 0 on success.  Returns the same error codes on failure as
 * `futex_wake`.  This function can fail only if you do not have enough stack
 * or trusted stack to call `futex_wake`.
 */
...
__always_inline static inline int run_once(struct OnceState *state,
                                           void (*callback)(__if_c(void)))
{
	/**
	 * Helper function that implements the slow path for `run_once`.  This
	 * should not be called explicitly and is prototyped locally to avoid
	 * accidentally exposing it elsewhere.
	 */
	int __cheriot_libcall run_once_slow_path(struct OnceState * state,
	                                         void (*callback)(__if_c(void)));
	// Fast path, if intitialisation has run then we can elide the call with a
