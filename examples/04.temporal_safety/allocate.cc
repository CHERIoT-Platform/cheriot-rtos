...
int __cheri_compartment("allocate") entry()
{
	...
	// shows the more typical use case
	{
		...
		Debug::Invariant(make_claim(x) >= 0, "Unable to call make_claim");
		...
		Debug::Invariant(show_claim() >= 0, "Unable to call show_claim");

		// Give the claimant another ptr so it releases the first
		void *y = malloc(10);
		Debug::Invariant(make_claim(y) >= 0, "Unable to call make_claim");
