...
Spi_Config cfgCan1;
MCP251XFD_BitTimeStats can0Bittimestats;
uint32_t sysclkExt1;
MCP251XFD can1 = {
...
MCP251XFD_RAMInfos ext1TefRamInfos;
MCP251XFD_RAMInfos ext1TxqRamInfos;
MCP251XFD_RAMInfos ext1FifosRamInfos[MCP251XFD_EXT1_FIFO_COUNT - 2];
MCP251XFD_FIFO mcP251XfdExt1FifOlist[MCP251XFD_EXT1_FIFO_COUNT] =
{
...
{
 { .Filter = MCP251XFD_FILTER0, .EnableFilter = true, .Match = MCP251XFD_MATCH_ONLY_SID, .PointTo = MCP251XFD_FIFO1, .AcceptanceID = 0x000, .AcceptanceMask = 0x600, }, // 0x000..0x1FF
 { .Filter = MCP251XFD_FILTER1, .EnableFilter = true, .Match = MCP251XFD_MATCH_ONLY_SID, .PointTo = MCP251XFD_FIFO2, .AcceptanceID = 0x200, .AcceptanceMask = 0x600 }, // 0x200..0x3FF
 { .Filter = MCP251XFD_FILTER2, .EnableFilter = true, .Match = MCP251XFD_MATCH_ONLY_SID, .PointTo = MCP251XFD_FIFO3, .AcceptanceID = 0x400, .AcceptanceMask = 0x600 }, // 0x400..0x5FF
 { .Filter = MCP251XFD_FILTER3, .EnableFilter = true, .Match = MCP251XFD_MATCH_ONLY_SID, .PointTo = MCP251XFD_FIFO4, .AcceptanceID = 0x600, .AcceptanceMask = 0x600 }, // 0x600..0x7FF
};
...
//=============================================================================
eERRORRESULT receive_message_from_ext1_fifo(eMCP251XFD_FIFO fifo)
{
	eERRORRESULT ret = ERR_OK;
	eMCP251XFD_FIFOstatus fifoStatus = MCP251XFD_RX_FIFO_EMPTY;
	...
	if ((fifoStatus & MCP251XFD_RX_FIFO_NOT_EMPTY) > 0) // Second check FIFO not empty
	{
		...
		ret = MCP251XFD_ReceiveMessageFromFIFO(&can1, &receivedMessage, MCP251XFD_PAYLOAD_64BYTE,
