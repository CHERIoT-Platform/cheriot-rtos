...
We should make the epoch counter a futex: updates to the realtime clock are infrequent and it's useful to be able to watch for when the wall-clock time changes.
...
In particular, consider an implementation that reads the current wall-clock time and then computes the displacement to the desired time, then waits until a time in the monotonic clock.
In between reading the wall-clock time and waiting, the wall-clock time may change.
...
Devices should look somewhat like this (the implementation should define a concept to embody this in `platform/concepts`):
...
struct ExampleClockSource
{
	 ...
	 * 
	 * The units for the real time are microseconds, relative to the UNIX
	 * epoch.  The units for the monotonic time are defined by the platform's
	 * monotonic clock.
	 * 
...
Additional clocks.
POSIX defines a per-process CPU clock (which would likely mean per-thread on CHERIoT).
We could add such a thing later, but it would have a different implementation (the scheduler already tracks much of this information).
